...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	glm::mat4 tempmatrix;
	glm::mat4 bone_obj_mat;

	int currentpose;
	int nextpose;
	float progresstonextpose;

	// Draw bones
	for (int z = 0; z < m_bone_animation->rotation_degree_vector.size(); z++)
	{
		for (int i = 0; i < m_bone_animation->scale_vector.size(); i++)
		{
			tempmatrix = glm::mat4(1.0f);
			tempmatrix = glm::scale(glm::mat4(1.0f), m_bone_animation->scale_vector[i]);

			bone_obj_mat = glm::mat4(1.0f);
			bone_obj_mat = tempmatrix * bone_obj_mat;


			tempmatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0, (m_bone_animation->scale_vector[i].y / 2.0f), 0));
			bone_obj_mat = tempmatrix * bone_obj_mat;


			for (int r = i; r >= 0; r--)
			{
				tempmatrix = rotate(glm::mat4(1.0f), glm::radians(m_bone_animation->rotation_degree_vector[z][r].z), glm::vec3(1, 0, 0));
				bone_obj_mat = tempmatrix * bone_obj_mat;
				tempmatrix = rotate(glm::mat4(1.0f), glm::radians(m_bone_animation->rotation_degree_vector[z][r].x), glm::vec3(0, 1, 0));
				bone_obj_mat = tempmatrix * bone_obj_mat;
				tempmatrix = rotate(glm::mat4(1.0f), glm::radians(m_bone_animation->rotation_degree_vector[z][r].y), glm::vec3(0, 0, 1));
				bone_obj_mat = tempmatrix * bone_obj_mat;
				if (r == 0)
				{
					tempmatrix = glm::translate(glm::mat4(1.0f), m_bone_animation->root_position[z]);
					bone_obj_mat = tempmatrix * bone_obj_mat;
				}
				else
				{
					tempmatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0, m_bone_animation->scale_vector[r - 1].y, 0));
					bone_obj_mat = tempmatrix * bone_obj_mat;
				}
			}
		...
		currentpose = m_bone_animation->progresstracker / m_bone_animation->divisor;
		nextpose = currentpose + 1;
		if (nextpose >= m_bone_animation->poses.size())
		{
			nextpose = 0;
		}
		progresstonextpose = m_bone_animation->progresstracker % m_bone_animation->divisor;
		progresstonextpose /= (float)m_bone_animation->divisor;
		for (int i = 0; i < m_bone_animation->rotation_degree_vector[z].size(); i++)
		{
			m_bone_animation->rotation_degree_vector[z][i] = {
			m_bone_animation->poses[currentpose][z][i].x + (m_bone_animation->poses[nextpose][z][i].x - m_bone_animation->poses[currentpose][z][i].x) * progresstonextpose,
			m_bone_animation->poses[currentpose][z][i].y + (m_bone_animation->poses[nextpose][z][i].y - m_bone_animation->poses[currentpose][z][i].y) * progresstonextpose,
			m_bone_animation->poses[currentpose][z][i].z + (m_bone_animation->poses[nextpose][z][i].z - m_bone_animation->poses[currentpose][z][i].z) * progresstonextpose,
			};
		}
	}

	m_bone_animation->progresstracker++;
	if (m_bone_animation->progresstracker >= m_bone_animation->divisor * m_bone_animation->poses.size())
	{
		m_bone_animation->progresstracker -= m_bone_animation->divisor * m_bone_animation->poses.size();
	}
	
	tempmatrix = glm::mat4(1.0f);
	tempmatrix = glm::scale(glm::mat4(1.0f), m_bone_animation->body_scaling );

	bone_obj_mat = glm::mat4(1.0f);
	bone_obj_mat = tempmatrix * bone_obj_mat;
	
	tempmatrix = glm::translate(glm::mat4(1.0f), m_bone_animation->body_position );
	bone_obj_mat = tempmatrix * bone_obj_mat;
	
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_obj_mat));
	bone_obj->obj_color = m_bone_animation->body_color;
	draw_object(shader, *bone_obj);
